...
; Last modification date: 3/25/15
...
cstep1	LDR R2,=SSI0_SR_R				; R2 points to SSIO_SR_R
		LDR R1, [R2]
		MOV R3, #0x10					; bit 4
		AND R1, R1, R3
		CMP R1, #0						; If bit 4 is high (1)
		BNE cstep1
		
		LDR R2,=DC						; R2 points to DC
		MOV	R1, #0
		STR R1, [R2]
...
cstep5	LDR R1,=SSI0_SR_R				; R2 points to SSIO_SR_R
		LDR R1, [R1]
		MOV R2, #0x10					; bit 4
		AND R1, R1, R2
		CMP R1, #0						; If bit 4 is high (1)
...
dstep1	LDR R2,=SSI0_SR_R				; R2 points to SSIO_SR_R
		LDR R1, [R2]
		LDR R2, =SSI_SR_TNF				; bit 1
		ANDS R1, R2
		BEQ dstep1
		...
		LDR R1,=SSI0_DR_R			
		STR R0, [R1]					; write 8-bit data to SSI0_DR_R
		BX  LR                          ; return
